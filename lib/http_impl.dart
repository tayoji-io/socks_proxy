...
class _HttpProfileData {
  ...
  static final String curIsolateId = Service.getIsolateID(Isolate.current)!;
...
class _HttpClientConnection {
  ...
  Future<_HttpClientConnection> createProxyTunnel(
      ...
      _HttpProfileData? profileData) {
    if (proxy.socksType != _SocksType.None) {
      return createSocksProxyTunnel(host, port, proxy, callback, profileData);
    }
    ...
    if (profileData != null) {
      proxyProfileData = _HttpProfiler.startRequest(
        method,
        uri,
  ...
  Future<_HttpClientConnection> createSocksProxyTunnel(
      ...
      _Proxy proxy,
      bool callback(X509Certificate certificate),
      _HttpProfileData? profileData) async {
    final username = proxy.username ?? '';
    final password = proxy.password ?? '';
    final uriPortBytes = [(port >> 8) & 0xFF, port & 0xFF];
    final uriAuthorityAscii = ascii.encode(host);
    _httpParser._socksType = proxy.socksType;
    _httpParser._socksState = _SocksState.Auth;
    if (proxy.socksType == _SocksType.Socks4) {
      _socket.add([
        0x04, // SOCKS version
      ...
      await _httpParser._socksStateController.stream.firstWhere((a) {
        ...
        return a == _SocksState.RequestReady;
      }).then((value) {
        var addressType = _SocksAddressType.Domain;
        final ipv4 = RegExp(
            r'^((25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(25[0-5]|2[0-4]\d|[01]?\d\d?)$');
        final ipv6 = RegExp(r'^([\da-fA-F]{1,4}:){7}[\da-fA-F]{1,4}$');
        if (ipv4.hasMatch(host)) {
          addressType = _SocksAddressType.IPv4;
        } else if (ipv6.hasMatch(host)) {
          addressType = _SocksAddressType.IPv6;
        }
        _socket.add([
          0x05,
          0x01, // 0x01 Connect; 0x02 Bind; 0x03 UDPAssociate;
          0x00,
          addressType.value,
          if (addressType == _SocksAddressType.Domain) uriAuthorityAscii.length,
          ...uriAuthorityAscii,
          ...uriPortBytes
        ]);
      });
    }
    return _httpParser._socksStateController.stream
        .firstWhere((a) => a == _SocksState.Connected)
...
class _HttpClient implements HttpClient {
  ...
  Future<_HttpClientRequest> _openUrl(String method, Uri uri) {
    ...
    if (HttpClient.enableTimelineLogging) {
      profileData = _HttpProfiler.startRequest(method, uri);
    }
...
class _ProxyConfiguration {
  ...
  static const String SOCKS4_PREFIX = "SOCKS4 ";
  static const String SOCKS5_PREFIX = "SOCKS5 ";
  ...
  _ProxyConfiguration(String configuration) : proxies = <_Proxy>[] {
    ...
    list.forEach((String proxy) {
      ...
      if (!proxy.isEmpty) {
        if (proxy.startsWith(PROXY_PREFIX)) {
        ...
        } else if (proxy.startsWith(SOCKS4_PREFIX)) {
          // Skip the "SOCKS4 " prefix.
          proxy = proxy.substring(SOCKS4_PREFIX.length).trim();
          ...
          proxies.add(_Proxy(
            ...
            null,
            null,
            socksType: _SocksType.Socks4,
          ));
        } else if (proxy.startsWith(SOCKS5_PREFIX)) {
          // Skip the "SOCKS5 " prefix.
          proxy = proxy.substring(SOCKS5_PREFIX.length).trim();
