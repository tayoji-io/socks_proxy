...
class _HttpParser extends Stream<_HttpIncoming> {
  ...
  _SocksType _socksType = _SocksType.None;
  _SocksState _socksState = _SocksState.Starting;
  final StreamController<_SocksState> _socksStateController =
  ...
  void _doParse() {
    ...
    while (_buffer != null &&
      ...
      if (_socksType != _SocksType.None &&
          _socksState != _SocksState.Connected) {
        SocksException? exception;
        if (_socksType == _SocksType.Socks4) {
          _index = _buffer!.length;
          if (_buffer!.length == 8) {
            if (_buffer![0] == 0x00) {
              if (_buffer![1] == 0x5A) {
                _socksState = _SocksState.Connected;
                _socksStateController.add(_SocksState.Connected);
                break;
              }
              if (_buffer![1] == 0x5B ||
                  _buffer![1] == 0x5C ||
                  _buffer![1] == 0x5D) {
                exception = SocksException.failure;
              }
            }
          }
        } else {
          if (_socksState == _SocksState.Auth) {
            if (_buffer!.length == 2) {
              final auth = _buffer![1];
              _index = _buffer!.length;
              if (auth == _SocksAuthMethod.UsernamePassword.value) {
                _socksState = _SocksState.AuthStarted;
                _socksStateController.add(_SocksState.AuthStarted);
                break;
              } else if (auth == _SocksAuthMethod.NoAuth.value) {
                _socksState = _SocksState.RequestReady;
                _socksStateController.add(_SocksState.RequestReady);
                break;
              } else if (auth == _SocksAuthMethod.NoAcceptableMethods.value) {
                exception = SocksException.noAuthMethods;
              }
            } else {
              exception = SocksException.failure;
            }
          } else if (_socksState == _SocksState.AuthStarted) {
          ...
          } else if (_socksState == _SocksState.RequestReady) {
            _index = _buffer!.length;
            if (_buffer!.length >= 10) {
              final reply = _SocksReply._(_buffer![1]);
              if (reply._value == _SocksReply.Success._value) {
                _socksState = _SocksState.Connected;
                _socksStateController.add(_SocksState.Connected);
                break;
              } else {
                exception = SocksException._(reply.toString());
              }
            }
          }
        }
        _socksStateController.addError(exception ?? SocksException.failure);
        break;
      }
